...
RUN apt-get update && \
    apt-get install -y ca-certificates wget tar gnupg python3 --no-install-recommends && \ 
    apt-get clean && \
    rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*

# # Download and extract Bitcoin Core binaries
# RUN wget https://bitcoincore.org/bin/bitcoin-core-${BITCOIN_VERSION}/bitcoin-${BITCOIN_VERSION}-${PLATFORM_ARCH}.tar.gz && \
...
RUN tar -xzvf "${TMPDIR}.${BITCOIN_VERSION}-linux/bitcoin-${BITCOIN_VERSION}-${PLATFORM_ARCH}.tar.gz" --strip-components=1 -C /btc && \
    rm "${TMPDIR}.${BITCOIN_VERSION}-linux/bitcoin-${BITCOIN_VERSION}-${PLATFORM_ARCH}.tar.gz"
